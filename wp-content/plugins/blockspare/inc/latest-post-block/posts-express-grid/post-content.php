...
function blockspare_express_post_image($attributes,$post_id,$cat_class ,$blockName='',$className='',$count=''){
    ...
    $spotlightItems = 0;
    if ($attributes['express']== 'blockspare-posts-block-express-grid-layout-3') {
        $spotlightItems = 1;
               ...
               ?>
                   <figure class="blockspare-posts-block-post-img">
                       <a href="<?php echo esc_url(get_permalink($post_id));?>" rel="bookmark" aria-hidden="true"
                           ...
                           echo wp_kses_post(wp_get_attachment_image($post_thumb_id, $attributes['spotslightImageSize'], '', ["class" => 'hover-child']));
                               }
                               if($count > $spotlightItems){
