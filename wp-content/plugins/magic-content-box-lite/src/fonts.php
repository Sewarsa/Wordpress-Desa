...
if ( ! class_exists( 'magic_content_box_lite_liteGoogle_Fonts' ) ) {
    class magic_content_box_lite_Google_Fonts {
		...
		public function enqueue_frontend_block_fonts() {
			...
			if ( ! apply_filters( 'magic_content_box_lite_enqueue_fonts', true ) ) {
				return;
			}
		...
		public function is_magic_content_box_lite_block( $block_name ) {
			return strpos( $block_name, 'magic-content-box/' ) === 0;
		}
		...
		public function gather_google_fonts( $blocks ) {
			...
			foreach ( $blocks as $block ) {
				...
				if ( $this->is_magic_content_box_lite_block( $block['blockName'] ) ) {
		...
		public function enqueue_google_fonts( $google_fonts ) {
			...
			wp_enqueue_style( 'magic-content-box-lite-google-fonts', $fonts_url ); // phpcs:ignore WordPress.WP.EnqueuedResourceParameters.MissingVersion
		}

	}

	new magic_content_box_lite_Google_Fonts();
}
